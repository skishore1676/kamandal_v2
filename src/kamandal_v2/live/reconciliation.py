...
def reconcile_live_positions(
    ...
    *,
    ...
    for group in local_groups:
        ...
        if issue:
            stored = _record_issue(config, store, issue, dry_run=dry_run)
...
def _record_issue(config: dict[str, Any], store: LocalStore, issue: dict[str, Any], *, dry_run: bool) -> dict[str, Any]:
    ...
    if _should_auto_retire(config, stored):
        ...
        if group:
            ...
            store.update_live_reconciliation_issue_status(str(stored["issue_id"]), "retired", {"resolution": "auto_retired_after_flat_confirmations", "retired_at": _now()})
