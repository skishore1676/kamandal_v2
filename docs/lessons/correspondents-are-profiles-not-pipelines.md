...
A dedicated source can also appear in a generic timeline/bookmark feed. Source ownership must exclude that author from generic extraction, including when its dedicated lane is off/observe. Otherwise classification and effect ceilings can be bypassed. An observation projection must also preserve the idea row when exact evidence fails and join lifecycle state by distinct idea/revision identities, including closed lifecycles. Retained evidence can be reprojected without invoking models or trading jobs.
